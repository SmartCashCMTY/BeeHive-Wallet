...
import { SMARTCASH3 } from 'application/config/smartcash3';
...
export async function getEnabledNodes() {
    try {
        ...
        const servers = SMARTCASH3.sapiServers;

        const fasterNodes = await testNodeResponseTime(servers);
        const enabledNodes = fasterNodes.length ? fasterNodes : servers;

        window.sessionStorage.setItem(SAPI_SERVERS_KEY, JSON.stringify(enabledNodes));

        return new Promise((resolve, reject) => resolve(enabledNodes));
    } catch (err) {
        console.error(err);
        return SMARTCASH3.sapiServers;
    }
}
...
async function getEnabledNode(sapis) {
    if (!sapis || !sapis.length) return SMARTCASH3.sapiServers[0];

    var electedSapi = sapis[random.int(0, sapis.length - 1)];
    const res = await ping(electedSapi);

    if (!res) {
        const remainingSapis = sapis.filter((sapi) => sapi !== electedSapi);
        return await getEnabledNode(remainingSapis);
    }
...
export async function getTransactionHistoryFromMemoryPool(address) {
    try {
        const transactions = await request.get(`${await GetSapiUrl()}/v1/address/mempool/${address}`, {
            json: true,
        });
...
export async function getNodesUrl() {
    try {
        var options = {
            method: 'GET',
            uri: `${await GetSapiUrl()}/v1/smartnode/check/ENABLED`,
            json: true, // Automatically stringifies the body to JSON
        };
